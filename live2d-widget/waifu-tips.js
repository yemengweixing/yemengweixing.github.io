		...
		fetch(waifuPath)
			...
			.then(result => {
				...
				result.seasons.forEach(tips => {
					...
					if ((after.split("/")[0] <= 1="" now.getmonth()="" +="" &&="" <="before.split("/")[0])" (after.split("="" ")[1]="" now.getdate()="" {="" var="" text="Array.isArray(tips.text)" ?="" tips.text[math.floor(math.random()="" *="" tips.text.length)]="" :="" tips.text;="" now.getfullyear());="" showmessage(text,="" 7000,="" true);="" messagearray.push(text);="" }="" });="" initmodel();="" function="" loadmodel(modelid,="" modeltexturesid)="" localstorage.setitem("modelid",="" modelid);="" if="" (modeltexturesid="==" undefined)="" modeltexturesid="0;" localstorage.setitem("modeltexturesid",="" modeltexturesid);="" loadlive2d("live2d",="" `${apipath}="" get="" ?id="${modelId}-${modelTexturesId}`," console.log(`live2d="" 模型="" ${modelid}-${modeltexturesid}="" 加载完成`));="" loadrandmodel()="" modelid="localStorage.getItem("modelId")," 可选="" "rand"(随机),="" "switch"(顺序)="" fetch(`${apipath}="" rand_textures="" .then(response=""> response.json())
			.then(result => {
...
function initWidget(waifuPath = "/waifu-tips.json", apiPath = "") {
	if (screen.width <= 768)="" return;="" document.body.insertadjacenthtml('beforeend',="" `<div="" id="waifu-toggle" style="margin-left: -100px;">
			<span>看板娘</span>
		`);
	$("#waifu-toggle").hover(() => {
	...
	if (localStorage.getItem("waifu-display") && Date.now() - localStorage.getItem("waifu-display") </=></=></=></=></=></=></=></=></=></=>